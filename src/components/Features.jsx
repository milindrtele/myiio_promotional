function FeatureIcons() {
  ...
  return (
    <section className="w-full bg-white py-8 px-4 bg-[repeating-linear-gradient(135deg,#ccc_0px,#ccc_2px,transparent_2px,transparent_8px)] ">
      <div className="grid grid-cols-4 md:grid-cols-8 gap-4 justify-center items-center w-[90%] md:w-[75%] m-auto">
        {icons.map((icon, i) => (
          <div
            key={i}
            className="w-22 md:w-28 h-28 md:w-34 md:h-34 flex items-center justify-center m-0"
          >
            <img
              src={`/assets/${icon}`}
              alt={icon}
